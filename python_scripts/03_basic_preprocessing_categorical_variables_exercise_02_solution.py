...
# approximately the same as the reference pipeline (any time difference you
# might observe is not significant).
#
# Scaling numerical features is indeed useless for most decision tree models
# in general and for `HistGradientBoostingClassifier` in particular.
...
# enough to deal with misleading ordering of integer coded categories (which
# was not the case for linear models).
#
...
#
# Note that the current implementation `HistGradientBoostingClassifier` is
# still incomplete, and once sparse representation are handled correctly,
# training time might improve with such kinds of encodings.
#
# The main take away message is that arbitrary integer coding of categories
# is perfectly fine for `HistGradientBoostingClassifier` and yields fast
# training times.